...
namespace Capsara.SDK.Tests.Internal
{
    ...
    public class RetryHandlerTests
    {
        ...
        public async Task SendAsync_429ThenSuccess_Retries()
        {
            // Arrange
            var mockHandler = new MockRetryHandler();
            // 429 requires a Retry-After header to trigger retry; without it, it's a hard block
            mockHandler.AddResponse(req =>
            {
                ...
                resp.Content = new StringContent("", System.Text.Encoding.UTF8, "application/json");
                return resp;
            });
            mockHandler.AddResponse(HttpStatusCode.OK, "{\"data\": \"test\"}");
        ...
        [Fact]
        public async Task SendAsync_ServerSuggestedDelayExceedsMax_UsesServerDelay()
        {
            // Arrange — server-suggested delays are respected WITHOUT capping at maxDelay
            var mockHandler = new MockRetryHandler();
            mockHandler.AddResponse(HttpStatusCode.ServiceUnavailable,
                "{\"error\": {\"retryAfter\": 1}}"); // 1 second delay from server
            mockHandler.AddResponse(HttpStatusCode.OK);
            ...
            var retryHandler = new RetryHandler(mockHandler, new RetryConfig
            {
                MaxRetries = 3,
                BaseDelay = TimeSpan.FromMilliseconds(10),
                MaxDelay = TimeSpan.FromMilliseconds(100) // maxDelay is 100ms but server says 1s
            });
            var client = new HttpClient(retryHandler);
            ...
            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);
            // Server delay of 1s is NOT capped at maxDelay (100ms) — full server delay is used
            elapsed.TotalMilliseconds.Should().BeGreaterThan(800);
        }
