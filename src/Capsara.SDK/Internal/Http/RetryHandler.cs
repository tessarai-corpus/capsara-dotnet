...
namespace Capsara.SDK.Internal.Http
{
    ...
    internal sealed class RetryHandler : DelegatingHandler
    {
            ...
            while (true)
            {
                try
                {
                    ...
                    if ((int)response.StatusCode == 429 &&
                        ...
                        !await HasBodyRetryAfterAsync(response).ConfigureAwait(false))
                    {
                        return response;
                    }
        ...
        private static async Task<bool> HasBodyRetryAfterAsync(HttpResponseMessage response)
        {
            var delay = await GetServerSuggestedDelayAsync(response).ConfigureAwait(false);
